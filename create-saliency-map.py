...
def createSaliencyMap(parsedPoints, path, imageData, outputPath):
    ...
    for i in range(len(scaledPoints)):
        ...
        radius = int(duration / 80)
        brightness = duration / 20
...
def parseSurveyResults(keys):
    ...
    for k in keys:
        ...
        parsed = SurveyData(data['gender'], data["age"], data["doYouHaveVisualImpairment"], data["visualImpairmentType"])
...
def createSaliencyMaps():
    ...
    for uuid in parsedSurveyResults:
        results = getResultsFromUser(uuid)
        currentPath = OUTPUT_PATH + "/" + parsedSurveyResults[uuid].hasImpairment
        makeDir(currentPath)
        currentPath = currentPath + "/" + uuid
        makeDir(currentPath)
        createTagFile(currentPath, parsedSurveyResults[uuid])
        for r in results:
            key = r['Key']['S']
            imageId = r['ImageId']['S']
            imageName = getImagePath(imageId)
            imageFolder = imageName.split("/")[0]
            makeDir(currentPath + "/" + imageFolder)
            ...
            try:
                ...
                createSaliencyMap(parsedPoints, imagePath, fixationPoints.imgData, currentPath + "/" + imageName)
            except Exception as e:
                print("Failed to create saliency map for " + key + " | " + repr(e))
            print("done")
